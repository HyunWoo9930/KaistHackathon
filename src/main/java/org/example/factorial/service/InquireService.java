...
public class InquireService {
	...
	public InquireAnswerResponse saveResponse(UserDetails userDetails, String answerText, Long inquireId) throws
		...
		return new InquireAnswerResponse(save.getInquireId(), save.getInquireText(), save.getAnswerText(),
			save.getCreatedAt());
	}
	...
	public List<InquireAnswerResponse> getAllResponses() {
		return inquireRepository.findAll().stream().map(inquire -> new InquireAnswerResponse(
			inquire.getInquireId(), inquire.getAnswerText(), inquire.getInquireText(), inquire.getCreatedAt()
		)).toList();
	}
}
